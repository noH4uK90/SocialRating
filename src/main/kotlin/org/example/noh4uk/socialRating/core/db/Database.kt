...
interface Database {
    ...
    fun createSQLiteTableIfNotExists() {
    ...
    fun createMySQLTableIfNotExists() {
        val connection = getConnection()
        connection.autoCommit = false
        connection.use { conn ->
            val statement = conn.createStatement()
            val players = statement.execute("""
                CREATE TABLE IF NOT EXISTS player_rating (
            ...
            """)
            SocialRating.getInstance().logger.info("Tables players: $players, history: $history")
            connection.commit()
        }
    }
}